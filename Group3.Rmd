...
while(R > K){
  ...
  names <-names[order(w, decreasing=TRUE)]
  
  # indsort <- rank(desc(w), ties="first")
  # for(i in (1:length(names))) { 
  #   names[i] <- names[which(indsort==1)]
  # }
  m <- as.integer(sqrt(R))
  w <- w_sort[1:(length(w_sort)-m)] 
  names <- names[1:(length(names)-m)]
  ...
  test_pred <- predict(svmfit, newdata=cbind(factor(label_test),df_test), decision.values = FALSE)
  confusionMatrix(test_pred, factor(label_test))
  R <- ncol(df)-1
}
...
#   
#     svmfit <- svm(Group ~ ., data = df, kernel = "linear", type = 'C-classification', scale = FALSE)
#     #manca da capire qual è quella migliore da togliere (nelle slide parla di C, che è l'accuracy) e poi aggiornare i valori
#   }
# }
```
