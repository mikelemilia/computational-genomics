...
### CHECK SUL COMMENTO
<div style="text-align: justify">
In the following chunks, thanks to the PCA(...) function we extract the first 10 PCs from the data w.r.t. samples and genes, using the "cos2" coloration to highlight the importance of the PC on a given observation. We can also see thanks to the Scree plot that the maximum variation in the data is captured within the first component, as we expected.
</div>
...
plot(samples[,1], samples[,12], pch=20, cex=1,  col="#3185FC",xlab="Sample 1", ylab="Sample 12", main = "Samples plot")

plot(extracted, selected, pch=20, cex=1,  col="#3185FC", xlab="Sample 1", ylab="Sample 12", main = "Samples plot - Normalized")
```
...
plot(computed$A, computed$M, pch=20, cex=1,  col="#3185FC",xlab="A", ylab="M", main = "MvA plot - TMM ", sub = paste("Sample", 1, "vs.", 12, sep = " "))
abline(0,0)
...
plot(computed$A, computed$M, pch=20, cex=1,  col="#3185FC",xlab="A", ylab="M", main = "MvA plot - Quantile", sub = paste("Sample", 1, "vs.", 12, sep = " "))
abline(0,0)
```
...
fviz_dend(cut(hcd_G, h=400)$lower[[1]], k = kopt_g_hier_sil, cex = 0.5, k_colors = c("#2E9FDF", "#00AFBB", "#E7B800", "#FC4E07"),
             color_labels_by_k = T, rect = F, show_labels = T, horiz = T)  

fviz_dend(cut(hcd_G, h=400)$lower[[2]], k = 2, cex = 0.5, k_colors = c("#2E9FDF", "#00AFBB", "#E7B800", "#FC4E07"),
             color_labels_by_k = T, rect = T, show_labels = F, horiz = T)  

# SAMPLES, HIERARCHICAL CLUSTERING
hcd_S <- as.dendogram(cl_hclust_ward_S)
fviz_dend(hcd_S, k = kopt_s_hier_sil, cex = 0.5, k_colors = c( "#E7B800", "#FC4E07", "#2E9FDF", "#00AFBB"), 
          color_labels_by_k = T, rect = T, rect_fill = T, show_labels = T)  
```

```{r plot of part 4, kmeans}
#GENES, KMEANS
cl_genes<-kmeans(x=dataNorm_clustering, centers = kopt_g_kmeans_sil, iter.max = 100, nstart = 100)
...
#SAMPLES, KMEANS
cl_samples<-kmeans(x=t(dataNorm_clustering), centers = kopt_s_kmeans_sil, iter.max = 100, nstart = 100)
...
w <- abs(w)
names_sorted <- colnames(dataNorm_test)[order(w, decreasing=TRUE)]
#print(names_sorted)
plot(svmfit, dataNorm_test, ENSG00000075673 ~ ENSG00000183665)
}# HIDE
```
...
#plot(rfe$number_features, rfe$accuracy)
```

<div style="text-align: justify">
With the best model returned by recursiveFeatureExtractionCV, we now evaluate its performances with the test set.
</div>
...
# come geni mettere i primi due tornati dalla riga precedente 
#plot(dataNorm_train[,"ENSG00000112212"],dataNorm_train[,"ENSG00000162006"],pch=19)
#dat = dataNorm_train[,"ENSG00000112212"],dataNorm_train[,"ENSG00000162006"])
plot(best_model, dataNorm_train, ENSG00000104722~ENSG00000162723, pch = 19)
```
