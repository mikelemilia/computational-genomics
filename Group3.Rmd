...
The function:
- first of all, we find all subjects duplicated in the group (in our case the group can be "control" or "disease"). To do so, we use the column of the group "individuals"; this column has the identifier for the subjects. If a sample comes from the same subject (i.e, the subject has benn sampled twice or more, so it is a duplicate), the identifier will be the same.
- We save all these dupes in a temporary matrix called "duplicate_group"
- we initialize a new matrix which will contain all subjects without any of them being duplicated (i.e., sampled twice)
- for each identifier in "duplicate_group"
    - find the indexes of this duplicate individual
    - get their SRR code (the code with which samples are identified in the matrix of raw data).         NB: in the matrix of raw data, all samples are identified with different SRR codes, but
      SRR codes can point to the same subject.
    - get all the the samples coming from this particular subject that we know is duplicated
    - apply the mean by row to all samples identified in the previous step and save this new             "mediated-samples subject" in the matrix initialized above
- then, find all subjects in the group that are not duplicated (i.e., there's a 1 to 1 correspondance of subject to sample for these individuals)
- put all these subjects not duplicated into the matrix initialized above
- remove from the matrix any columns not used (we initialized the matrix having the same columns as the group but obviously in the end it will have less columns because some samples will not be present anymore)
end.
</div>
...
<div style="text-align: justify">
For each test, we extracted the genes that have p-value lower than a fixed alpha. With the following function we calculate the expected values of true positives, false positives, true negatives and false negatives implementing their definition. We then applied the function with G0 (number of selected genes) equal to the total number of genes.
</div>
...
<div style="text-align: justify">
The first function calculates the values of G0 for a given set of lambda values. It's used to estimate the correct value of G0 rather than assuming that G0 is equal to the total number of genes. It receives in input also the p-values for which the computation must be done, the total number of genes G and the filename in which the plot of "G0 VS lambda" should be saved. From the plot we choose by hand the best value for our lambda (in this case 0.8) and with the second function, we provide an estimate of G0 with the previous lambda value passed as input (considering a mean of the values of G0 within an interval around the best lambda).
</div>
...
<div style="text-align: justify">
In the following chunk we perform the estimation of G0 with the functions described above; after seeing the plots, we choose lambda = 0.8 because we saw that around it the graph has a constant mean value with little variability.
</div>
...
dt <- cbind(c(G0_est_ttest,G0_est_wilcoxontest,G0_est_edger),dt)
rownames(dt)<-c('t-test', 'Wilcoxon-test', 'edgeR')
colnames(dt)<-c('G0 estimated','TP', 'FP', 'TN', 'FN')
dt %>% kbl() %>% kable_paper("hover", full_width = T)
```
...
<div style="text-align: justify">
For the selected genes, duplicates are removed as per the function implementd and explained above. This will the data to use for SVM classification; first of all, data is divided into training and test set randomly, saving the labels needed for the classification (the beloning to control or disease group). From both sets, genes selected but whose expression is zero both in disease and control group are taken out. When taking away said genes, it must be taken away also in the other set.They won't be of any use in the classification process; in fact, in the preprocessing of data, mean and standard deviations must be computed to be used for z-scoring said data; if a gene has always expression zero in all samples, its standard deviation will also be zero. Dividing by zero is not possibile, so NA will be inserted; the training will be stopped because NA will be found. For this reason, we implemented a function to remove those genes, in a similar fashion to the function for removing zeros explained above.
</div>
...
<div style="text-align: justify">
Z-scoring is implemented; to z-score the data set, mean and standard deviation of the training set is used. In this way, we are forcing that bot sets have the same distribution.
</div>
...
bestSVM <- svmfit
bestNames <- as.vector(colnames(df[,-1]))
bestAcc <- C


while(R > K && C[length(C)]>C[1]*0.9){
  ...
  svmfit_pre <- svmfit
  svmfit <- svm(Group ~ ., data = df, kernel = "linear", type = 'C-classification', scale = FALSE,na.action = na.omit)
  ...
  cat(R, "\t~", res[["overall"]][["Accuracy"]], "\n")
  R <- ncol(df)-1
  acc <- res[["overall"]][["Accuracy"]]
  
      cat("Accuracy obrained:", acc, ", features removed:", m, "\n")

  C_plot <- c(acc,C_plot)
  num_par_plot <- c(R,num_par_plot)
  
  if (C[length(C)]>(acc+eps)){
    svmfit <- svmfit_pre
    df <- df_pre
    df_test <- df_test_pre
    ...
    C <- c(acc,C)
    num_par <- c(R,num_par)
    if (acc >= bestAcc && R < length(bestNames)){
      bestSVM <- svmfit
      bestNames <- as.vector(colnames(df[,-1]))
      bestAcc <- acc
    }
...
plot (x=num_par_plot,y=C_plot)

cat("\nRemoving one feature in each iteration\n")

R <- num_par[1]
C0 <- C[1]   # needed for first cycle
...
bests <- list()
bestsnames <- list()
bests <- append(list(svmfit),bests)
bestsnames <- c(list(colnames(df[,-1])),bestsnames)

while(R >= 2){
  deltas <- NULL
  # svmfit_all <- vector (mode = 'list', length = R)
  ...
  for(i in 2:ncol(df)) {
    ...
    svmfit2 <- svm(Group ~ ., data = df2, kernel = "linear", type = 'C-classification', scale = FALSE, na.action = na.omit)
    pred2 <- predict(svmfit2, newdata=dataNorm_test, decision.values = FALSE)
    res2 <- confusionMatrix(pred2, factor(label_test))
    
    acc <- res2[["overall"]][["Accuracy"]]
    deltas <- c(deltas, (C0-acc))
  }
  ...
  # remove features
  df <- df[, -(idx+1)]
  df_test <- df_test[, -(idx+1)]
  
  svmfit <- svm(Group ~ ., data = df, kernel = "linear", type = 'C-classification', scale = FALSE, na.action = na.omit)
  pred <- predict(svmfit, newdata=dataNorm_test, decision.values = FALSE)
  res2 <- confusionMatrix(pred, factor(label_test))
  
  cat(R, "\t~", res2[["overall"]][["Accuracy"]], "\n")

  C0 <- res2[["overall"]][["Accuracy"]]
  R <- ncol(df)-1
  C2 <- c(C0,C2)
  num_par2 <- c(R,num_par2)
  names <- colnames(df[,-1])
...
for (i in (1:(length(C2)-1))){
  names <- bestsnames[[i]]
  df_prova <- dataNorm_train[,names]
  df_prova <- cbind(factor(label_train),df_prova)
  colnames(df_prova)[colnames(df_prova) == 'factor(label_train)'] <- 'Group'
  
  df_test_prova <- dataNorm_test[,names]
  df_test_prova <- cbind(factor(label_test),df_test_prova)
  colnames(df_test_prova)[colnames(df_test_prova) == 'factor(label_train)'] <- 'Group'
    
  svmfit_prova <- svm(Group ~ ., data = df_prova, kernel = "linear", type = 'C-classification', scale = FALSE, na.action = na.omit)
  pred_prova <- predict(svmfit_prova, newdata=df_test_prova, decision.values = FALSE)
  res_prova <- confusionMatrix(pred_prova, factor(label_test))
  
  g1 <- res_prova[["overall"]][["Accuracy"]]
  
  svmfit_prova <- bests[[i]]
  pred_prova <- predict(svmfit_prova, newdata=df_test_prova, decision.values = FALSE)
  res_prova <- confusionMatrix(pred_prova, factor(label_test))
  
  g2 <- res_prova[["overall"]][["Accuracy"]]
  
  g3 <- C2[i]
  
  cat("\ni:",i," --- g1:",g1," g2:",g2," g3:",g3)
}
...
# do some plot or analize some datas, the function return a list
```
