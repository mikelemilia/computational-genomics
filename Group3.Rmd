...
##--------
ng<-rep(c(-1, 1), c(21, 12)) #-1 for control, 1 for disease
datTRAIN <- data.frame(dataNorm_train, y = as.factor(ng))
svmfit <- svm(ng ~ ., data = datTRAIN, kernel = "linear", type = 'C-classification', 
             scale = FALSE)
print(svmfit)


ng<-rep(c(-1, 1), c(8, 4))
datTEST <- data.frame(dataNorm_test, y = as.factor(ng))
test_pred <- predict(svmfit, newdata=datTEST, decision.values = FALSE)
