...
computePCA ## questa funzione è l'implementazione dell'SVD suggerita dalla prof, è da riguarare, ad ogni modo non mi convincono i grafici delle PCA già implementate
...
# temp_pca <- prcomp(temp_samples, scale = FALSE)
# summary(temp_pca)
# fviz_pca_var(temp_pca, col.var = 'cos2', label = 'none', geom = 'point')
# fviz_pca_ind(temp_pca, col.var = 'cos2', label = 'none', geom = 'point')
```
...
library(kableExtra)
dt <- matrix(c(expected_ttest,expected_wilcoxontest,expected_edger),nrow=3)
...
<div style="text-align: justify">
In the following chunk we perform the estimation of G0 with the functions diescribed above; after seeing the plots, we choose lambda = 0.8 because we saw that around it the graph has a constant mean value with little variability.
</div>
...
# print results
dt <- matrix(c(expected_ttest_est,expected_wilcoxontest_est,expected_edgeR_est),nrow=3)
