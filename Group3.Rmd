...
```{r e1071 ADESSO FUNZIONA, con standardizzazione suggerita da prof}
## Standardize each feature in your dataset (so to have mean=0 and sd=1)

mean_train <- apply(data_train,2,mean)
sd_train <- apply(data_train,2,sd)
ind_NA_sd<-which(sd_train == 0)
data_train<-data_train[,-ind_NA_sd]
sd_train<-sd_train[-ind_NA_sd]
mean_train<-mean_train[-ind_NA_sd]
dataNorm_train <- scale(data_train, center = mean_train, scale = sd_train)
#which(is.na(dataNorm_train)) dovrebbe dare integer(0)
...
##--------
#-1 for control, 1 for disease
namegroup_int<-c(rep(-1, ncol(data_nodup_control)),rep(1, ncol(data_nodup_disease)))
ng<-namegroup_int[trainIndex]
dat <- data.frame(dataNorm_train, y = as.factor(ng))
svmfit <- svm(ng ~ ., data = dat, kernel = "linear", type = 'C-classification', 
             scale = FALSE)
print(svmfit)


ng<-namegroup_int[-trainIndex]
dat <- data.frame(dataNorm_test, y = as.factor(ng))
test_pred <- predict(svmfit, newdata=dat, decision.values = FALSE)



confusionMatrix(test_pred, factor(ng))
