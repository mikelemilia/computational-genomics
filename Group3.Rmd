...
```{r libraries, include = TRUE, echo = FALSE, results='hide'}
library(factoextra)
library(kableExtra)
...
depth <- apply(samples, 2, sum)     # 2 means that we're applying the sum by columns
```

```{r calculate depth output, echo=FALSE}
{# HIDE
...
<div style="text-align: justify">
The following function implements the Quantile Normalization. It receives in input the matrix x with samples in columns and genes in rows for which it computes the quantile normalization as defined in theory lessons. The setup matrixes for indexes, sorted values and normed values are created; then, for each subject, the gene raw counts of that subject are sorted and given a rank using the average method to deal with ties. Ranks will be used as indexes. Then, the mean by row is calculated on the matrix with sorted columns. At last,column by column, the normalized matrix is built using the indexes from the ranking function to choose which mean values should be put in which positions. 
...
plot(computed$A, computed$M, pch=20, cex=1,  col="#3185FC",xlab="A", ylab="M", main = "MvA", sub = paste("Sample", 1, "vs.", 12, sep = " "))
abline(0,0, col = "red")
...
<div style="text-align: justify">
As we said in the comment of the chunck above, quantile normalization seems to be more robust because produces a series of sample with the same distribution. This fact is also evident if you see the following plots, in fact after the application of the TMM normalization, there wasn't any effect on the PCs while using the quantile normalization you can clearly see the change in distribution of both samples and genes and the consequent reduction of the variance inside the data.
</div>

```{r Compute normalized PCA samples, results='hide', fig.width=7, fig.height=10}
# PCA for samples
...
```{r Compute normalized PCA genes, results='hide', fig.width=7, fig.height=10}
# PCA for genes
...
<li>Get all the samples coming from this particular subject that we know is duplicated</li>
...
<li>Then, find all subjects in the group that are not duplicated (i.e., there's a 1 to 1 correspondence of subject to sample for these individuals)</li>
...
```{r remove_duplicates outputs, echo = FALSE}
{# HIDE
# some outputs
cat("The number of control samples is:",nrow(control))
cat("\nThe number of disease samples is:",nrow(disease), "\n")

cat("\nThe number of control samples after duplicates removal is:",ncol(control_nodup))
cat("\nThe number of disease samples after duplicates removal is:",ncol(disease_nodup), "\n")
...
```{r remove_zeros outputs, echo=FALSE}
{# HIDE
# some outputs
cat("The number of initial genes is:",nrow(control_nodup), "\n")
cat("\nThe number of genes after zeros removal is:",nrow(control_nodup_nozero), "\n")
...
<div style="text-align: justify">
For the t-test and the Wilcoxon test, we use functions of the Stats library.
</div>
...
<div style="text-align: justify">
For each test, we extracted the genes that have p-value lower than a fixed alpha. These are the selected genes.
...
```{r selection for alpha 0.05 output, echo=FALSE}
{# HIDE
...
<div style="text-align: justify">
In the table below, we provide the results of the three tests in terms of FP, FN, TP, TN rates; moreover, we address sensitivity, specificity and accuracy of these tests. For better understanding, this analysis is done ultimately only for the results obtained by estimating G0.
...
```{r extraction output, echo=FALSE}
{# HIDE
# print results
...
```{r extract GO terms, results='hide'}
...
The function "annotation_terms" returns the annotated terms for the GOterms selected as meaningful; to do so it uses the GO.db databases.
</div>

```{r fisher_test_matrixes functions}
fisher_test_matrixes
```

```{r FDR_fisher functions}
FDR_fisher
```

```{r annotation_terms functions}
annotation_terms
```
...
<div style="text-align: justify">
To select the most important annotation terms we order the p-values produced by the Fisher Test from the lowest to the highest, taking into account only those with the lowest value.
</div>
...
<div style="text-align: justify">
We correct our data for the length of genes to perform a better clustering analysis and to not have a bias in the results due to the lengths. To do it we extracted the gene lengths with the 'getGeneLengthAndGCContent' function and then divide each value for its length. We also decided to consider as one sample all those samples taken from the same subject in the same group, as we did for testing the DE genes.
</div>
...
data_normalized <- t(t(data_normalized)/d[[1]]) # d[[1]] has the length of all genes selected
...
```{r results of the sihouette, echo=FALSE}
...
## Choose the optimal number of clusters using the Gap statistic

<div style="text-align: justify">
To implement the gap statistic, we used the clusGap() function of the library "cluster". For each analysis we implemented before, we determined the best number of clusters with its definition. 
</div>
...
```{r results of the gap statistic, echo=FALSE}
{# HIDE
...
<div style="text-align: justify">
After seeing results from both Gap and Silhouette Statistics, since both of them estimated the optimal number of clusters for genes at 2, we choose to use the Gap Statistic because it seems more robust to outliers present in the subjects group, considering that its task is to minimize the within cluster dispersion.
</div>

```{r plots of part4 hierarchical genes upper, fig.width=12, fig.height=7, results='hide'}
...
```{r plots of part4 hierarchical genes lower, fig.width=12, fig.height=7, results='hide'}
...
```{r plots of part4 hierarchical subjects, fig.width=12, fig.height=7, results='hide'}
...
fviz_dend(hcd_S, k = kopt_s_hier_gap, cex = 0.5, k_colors = c( "#E7B800",  "#00AFBB"), 
          color_labels_by_k = T, rect = T, rect_fill = F, show_labels = T, main="Hierarchical (Samples)")
```

```{r plot of part 4 kmeans genes, fig.width=12, fig.height=7}
#GENES, KMEANS
...
```{r plot of part 4 kmeans subjects, fig.width=12, fig.height=7}
#SUBJECTS, KMEANS
...
fviz_cluster(cl_samples, data = t(dataNorm_clustering_plot),
...
<div style="text-align: justify">
The dataset contains all data of the control and disease groups only for the genes selected with the differential expression analysis. To standardize each feature in the dataset (so to have mean=0 and sd=1), we used the scale function. Z-scoring is implemented: to z-score the data set, mean and standard deviation of the training set is used. We can perform the standardization of both sets with the values of the training set. This is done because in the reality, when a single new vector from a subject needs to be classified, it is not possible to use its mean and standard deviation (for each gene, we would have only one value) and it is so scaled with the already known information. 
...
```{r caret output train}
{# HIDE
# some results
cat("Prediction for the training set (caret):\n")
...
cat("Prediction for the test set (caret):\n")
...
```{r e1071 output}
{# HIDE
cat("Prediction for the test set (e1071):\n")
...
```{r funCV, results='hide'}
rfe <- recursiveFeatureExtractionCV(dataNorm_train, label_train, 500, 10)
```

```{r funCV plot, results='hide'}
plot (x = rfe$number_features, y = rfe$accuracy, xlab = "Features retained", ylab = "Accuracy")
```

<div style="text-align: justify">
With the best model returned by recursiveFeatureExtractionCV, we now evaluate its performances with the test set. In the first chunk, the best model is evaluated in terms of prediction on the test set (and a check on its performances on the entire training set). 
</div>
...
svmfit_2 <- svm(Group ~ ., data = matrix_train, kernel = "linear", type = 'C-classification', scale = FALSE, na.action = na.omit)
pred_2 <- predict(svmfit_2, newdata=matrix_train, decision.values = FALSE)
res_2 <- confusionMatrix(pred_2, factor(label_train))  
print(res_2)
```

<div style="text-align: justify">
It is possible to print the hyper plane generated by the best SVM model to separate the points of the training set. Since it can be visualized only in two dimensions, we considered as axis the two genes with higher weight in the model, so the most important ones. The hyper plane generated is not perfectly separating the two classes while in the four dimensional space it is a perfect linear separator for the training set: this is due to the projection that reduces the information reported in the plot.
</div>
...
plot(best_model, dataNorm_train, ENSG00000283504~ENSG00000175497, svSymbol = "v")
```

<div style="text-align: justify">
The genes selected as the ones that lead to the best model are now analyzed in terms of Gene Ontology, in order to find if they are associated with some important terms related with the colon tumor.
</div>

```{r GO annotations, results='hide'}
alldata <- select(org.Hs.eg.db, best_genes, columns = c("SYMBOL","ENTREZID", 
                  "ENSEMBL","GOALL"), keytype="ENSEMBL")
```

```{r GO annotations output}
terms <- unique(alldata[,4])
terms <- terms[!is.na(terms)]

annotation_featuresRFE <- as.data.frame(annotation_terms(vals, terms))
annotation_featuresRFE[1:10,]
```
...
The models obtained have in general a good accuracy over the test set: some of them have accuracy equal to one. These have an higher number of parameters with respect to the ones found as best model for the training set; they have not however a perfect accuracy over the training set and we imagine that they are able however to generalize in a good way the data. In addition, we can notice that many models have an high accuracy even if not perfect. We can conclude that data are well linearly separable and in presence of more subjects on which training the model, we could obtain a good and generalized model for the classification.
</div>

```{r accuracies of best models on test set}
accuracy_test <- NULL
par <- NULL
for (i in (1:length(rfe$bests))){
  svmfit_test <- rfe$bests[[i]]
  pred_test <- predict(svmfit_test, newdata=dataNorm_test, decision.values = FALSE)
  res_test <- confusionMatrix(pred_test, factor(label_test)) 
  accuracy_test <- c(accuracy_test, res_test[["overall"]][["Accuracy"]])
