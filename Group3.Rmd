...
data_train <- t(data_SVM)[trainIndex,]
data_test <- t(data_SVM)[-trainIndex,]
...
# library(e1071)
# 
# index <- 1:ncol(data_SVM)
# testindex <- sample(index, trunc(length(index)/3))
# data_test <- t(data_SVM)[testindex,]
# data_train <- t(data_SVM)[-testindex,]
# 
# label_train <- namegroup[-testindex]
# label_test <- namegroup[testindex]
...
```{r}
# svm_linear <- svm(group ~ ., data = dataNorm_train, cost = 10, gamma = 1)
# 
# prediction <- predict(svm_linear, dataNorm_test)
# 
# 
# table(prediction, factor(label_test))


group <- factor(label_train)

trctrl <- trainControl(method = "cv", number = 5)
svm_Linear <- train(group~., 
                    data = dataNorm_train, 
                    method = "svmLinear", 
                    trControl=trctrl)

prediction <- predict(svm_linear, dataNorm_test)
confusionMatrix(prediction, label_test)
...
```{r}
test_pred <- predict(svm_Linear, data_test)
confusionMatrix(test_pred, label_test)
```
