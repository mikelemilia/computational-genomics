...
```{r plots of part4 HIERARCHICAL, fig.width=12, fig.height=7, results='hide'}
#GENES, HIERARCHICAL CLUSTERING
hcd_G <- as.dendrogram(cl_hclust_ward)
#fviz_dend(cut(hcd_G, h=400)$lower[[1]], k = kopt_g_hier_gap, cex = 0.5, k_colors = c("#2E9FDF",  "#FC4E07"), color_labels_by_k = T, rect = F, show_labels = T, horiz = T, main="Hierarchical (Genes) - Upper")  
...
```{r plot of part 4 kmeans, fig.width=12, fig.height=7}
#GENES, KMEANS
...
The models obtained have in general a good accuracy over the test set: some of them have accuracy equal to one. These have an higher number of parameters with respect to the ones found as best model for the training set; they have not however a perfect accuracy over the training set and we imagine that they are able however to generalise in a good way the data. In addition, we can notice that many models have an high accuracy even if not perfect. We can conclude that data are well linearly separable and in presence of more subjects on which training the model, we could obtained a good and generalised model for the classification.
</div>
