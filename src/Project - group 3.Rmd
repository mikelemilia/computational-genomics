...
### It is to be noted that neither the quantile nor the TMM normalizations get rid of the "V-shaped" outliers near low values of A (average). The code below shows what was said here. 

```{r MvA plot of reference sample (sample 1) VS sample 12 with TMM and quantile normalization}

#quantile
par(mfrow=c(1,2)) 
extracted<-quantile_normed$samples[,2]
...
#TMM
extracted<-tmm_normed$samples[,2]
...
### 1.6 - Produce the MvA plots of each sample vs. sample 1 (or another sample of your choice) using normalized data to evaluate if the normalization step was correctly performed or if there are outlier samples.
...
### See MvA plots of reference sample VS sample 12 and reference sample VS 47; the plots of reference VS 12 was explained above. For reference VS 47 the same comments can be made: MvA plots clearly show a deviance from M=0 in TMM normalization while this deviance is not present in quantile normalizatio. This gives credit to the idea that quantile normalization is more robust because it produces a series of sample with the same distribution, while TMM normalization just takes care of the most evident outliers, because it uses a trimmed mean approach.
...
## Part 4
### 4.0 correcting for length of genes before clustering; we also decided to consider as one sample those samples taken for the same subject in the same group, as we did for testing the DE genes.
```{r init clustering}
