...
DATA2<-read.table(getRawPath("E-GEOD-76987-raw-counts.tsv"), sep = "\t", row.names = 1, header = TRUE)
...
cols <- ncol(DATA2[-1]) # get number of columns
...
dim <- dim(control)
control_nodup <- matrix(0,nrow(DATA2),dim[1])
#initialize a count because matrix at the end WON'T have same columns as dim[1]=41, the control SRR
count<-0
for (i in 1:length(duplicate_control))
{
  ...
  seq_sample <- control[indexes,]$seq.sample
  #mean of duplicated samples
  control_nodup[,i] <- apply(DATA2[, seq_sample], 1, mean)
  count <- count+1;
}
...
for (i in 1:length(ind))
{
  ...
  seq_sample<-control[ind[i],]$seq.sample
  #mean of duplicated samples
  control_nodup[,length(duplicate_control)+i]<-DATA2[,seq_sample]
  count<-count+1;
}
control_nodup<-control_nodup[,1:count]
